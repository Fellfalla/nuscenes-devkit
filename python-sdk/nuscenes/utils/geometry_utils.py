...
def box_in_image(box, intrinsic: np.ndarray, imsize: Tuple[int, int], vis_level: int = BoxVisibility.ANY):
    """
...
def points_in_box2(box: 'Box', points: np.ndarray, wlh_tolerance: float = 0.0, angle_tolerance: float = 0.0):
    ...
    mask_x = np.logical_and(min_wlh[0] <= rel_points[0,:] + angle_tolerances, rel_points[0,:] - angle_tolerances <= max_wlh[0])
    mask_y = np.logical_and(min_wlh[1] <= rel_points[1,:] + angle_tolerances, rel_points[1,:] - angle_tolerances <= max_wlh[1])
